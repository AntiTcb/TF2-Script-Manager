...
namespace TF2_Script_Manager.Services {
    ...
    public static class ConfigWriter {
        ...
        public static async Task WriteClassConfigFile(ClassConfig config) {
            Debug.Write(Core.Settings.TF2Directory + @"\custom\" + config.FileName);
            using ( var sw = new StreamWriter(Core.Settings.TF2Directory + @"\custom\" + config.FileName, false) )
            {
                ...
                if ( config.Binds.Count == 0 ) { return; }
                await sw.WriteLineAsync("// BINDS");
                foreach ( var bind in config.Binds ) { await sw.WriteLineAsync(bind.ToString()); }
            }
